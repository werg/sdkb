"""Compare frozen attention continuations on new common competing-entity worlds."""
import argparse
import json
    ...
    with run_lock(root, clear_stop=False):
        ...
        if not episodes.exists():
            save_episodes(episodes, [e for i in range(32) for e in make_multiuse_world(
                i, split='attention-context-confirmation-20260919', bindings=2)])
        ...
        for name, run in runs.items():
            ...
            if (output / 'summary.json').exists():
                ...
                evaluate(checkpoint, episodes, output)
