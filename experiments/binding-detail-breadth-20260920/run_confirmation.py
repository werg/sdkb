"""Wait for both committed endpoints, then evaluate three arms in separate processes."""
import argparse
import json
...
def main(root):
    ...
    while True:
        ready = True
        for name in ('worlds-128', 'worlds-1024'):
            stage = root/name
    ...
    children = []
    try:
        for name, source in [('source', Path(inputs['source'])), ('worlds-128', root/'worlds-128'),
                             ('worlds-1024', root/'worlds-1024')]:
